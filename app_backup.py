...
def init_db():
    ...
    # Poblar tablas maestras desde los tickets (para que luego se pueda desactivar)
    try:
        c.execute("SELECT DISTINCT branch FROM tickets WHERE branch IS NOT NULL AND TRIM(branch) != ''")
        for (b,) in c.fetchall():
            if b is None:
                continue
            name = str(b).strip()
