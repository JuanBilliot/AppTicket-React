...
def timeout(seconds):
    ...
    if platform.system().lower() == 'windows' or not hasattr(signal, 'SIGALRM'):
        yield
        return
    ...
    def get_data(self, ip):
        ...
        with self.lock:
            try:
                ...
                import copy
                ip_data = copy.deepcopy(self.data.get(ip, {}))
                ...
                latencies = ip_data.get('latencies') or []
                valid_latencies = [l for l in latencies if isinstance(l, (int, float)) and l > 0]
                if valid_latencies:
                    stats['min_latency'] = min(valid_latencies)
                    stats['max_latency'] = max(valid_latencies)
                    stats['avg_latency'] = sum(valid_latencies) / len(valid_latencies)
                else:
                ...
                return {
                    ...
                    'loss': ip_data.get('loss', 100),
                    'last_update': ip_data.get('last_update'),
                    'valid': ip_data.get('valid', True),
                    'stats': stats.copy()
                }
            except Exception as e:
...
def init_db():
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS user_branch_patterns (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    # Migración suave: si ya existían tablas previas sin columna active
    try:
        c.execute("PRAGMA table_info(branches)")
        branch_cols = [r[1] for r in c.fetchall()]
        if 'active' not in branch_cols:
            c.execute('ALTER TABLE branches ADD COLUMN active INTEGER NOT NULL DEFAULT 1')
    except Exception:
        pass
    ...
    try:
        ...
        agent_cols = [r[1] for r in c.fetchall()]
        if 'active' not in agent_cols:
            c.execute('ALTER TABLE agents ADD COLUMN active INTEGER NOT NULL DEFAULT 1')
    except Exception:
        pass

    # Agregar columna category a la tabla servers si no existe
    try:
        c.execute("PRAGMA table_info(servers)")
        server_cols = [r[1] for r in c.fetchall()]
        if 'category' not in server_cols:
        ...
        for (b,) in c.fetchall():
            ...
            name = str(b).strip()
            if name:
                c.execute('INSERT OR IGNORE INTO branches (name, active) VALUES (?, 1)', (name,))
    except Exception:
        pass

    try:
        c.execute("SELECT DISTINCT user FROM tickets WHERE user IS NOT NULL AND TRIM(user) != ''")
        for (u,) in c.fetchall():
            if u is None:
                continue
            name = str(u).strip()
            if name:
                c.execute('INSERT OR IGNORE INTO users (name, active) VALUES (?, 1)', (name,))
    except Exception:
        pass

    try:
        c.execute("SELECT DISTINCT agent FROM tickets WHERE agent IS NOT NULL AND TRIM(agent) != ''")
        for (a,) in c.fetchall():
            if a is None:
                continue
            name = str(a).strip()
            if name:
                c.execute('INSERT OR IGNORE INTO agents (name, active) VALUES (?, 1)', (name,))
    except Exception:
        pass
    ...
    try:
        ...
        for _, row in data.iterrows():
            ...
            if not c.fetchone():
                    ...
                    try:
                        ...
                        for fmt in ("%d/%m/%Y", "%Y-%m-%d", "%d-%m-%Y"):
                            try:
                                ...
                                close = datetime.strptime(close_date, fmt)
                ...
                # Mantener tablas maestras sincronizadas
                if branch.strip():
                    c.execute("INSERT OR IGNORE INTO branches (name, active) VALUES (?, 1)", (branch.strip(),))
                    c.execute("UPDATE branches SET active = 1 WHERE name = ?", (branch.strip(),))
                if user.strip():
                    c.execute("INSERT OR IGNORE INTO users (name, active) VALUES (?, 1)", (user.strip(),))
                    c.execute("UPDATE users SET active = 1 WHERE name = ?", (user.strip(),))
                if agent.strip():
                    c.execute("INSERT OR IGNORE INTO agents (name, active) VALUES (?, 1)", (agent.strip(),))
                    c.execute("UPDATE agents SET active = 1 WHERE name = ?", (agent.strip(),))
...
def api_get_agents():
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT name FROM agents WHERE active = 1 ORDER BY name")
...
def api_get_branches():
    ...
    all_branches = [r[0] for r in c.fetchall()]
    conn.close()
...
def api_settings_users():
    ...
    try:
        if request.method == 'GET':
            ...
            if include_inactive:
                c.execute('SELECT id, name, active FROM users ORDER BY name')
            else:
                c.execute('SELECT id, name, active FROM users WHERE active = 1 ORDER BY name')
            return jsonify([{'id': r[0], 'name': r[1], 'active': bool(r[2])} for r in c.fetchall()])
...
@app.route('/api/settings/users/<int:item_id>', methods=['PUT', 'DELETE'])
def api_settings_users_detail(item_id):
    conn = get_db()
    c = conn.cursor()
    try:
        c.execute('SELECT name FROM users WHERE id = ?', (item_id,))
        row = c.fetchone()
        ...
        if request.method == 'PUT':
            ...
            if new_name is not None:
                ...
                if not new_name:
                    return jsonify({'status': 'error', 'message': 'Nombre inválido'}), 400
                c.execute('UPDATE users SET name = ? WHERE id = ?', (new_name, item_id))
                if old_name:
                    c.execute('UPDATE tickets SET user = ? WHERE user = ?', (new_name, old_name))

            if active is not None:
                c.execute('UPDATE users SET active = ? WHERE id = ?', (1 if active else 0, item_id))

            conn.commit()
            return jsonify({'status': 'success'})

        c.execute('UPDATE users SET active = 0 WHERE id = ?', (item_id,))
        conn.commit()
        return jsonify({'status': 'success'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
@app.route('/api/settings/branches', methods=['GET', 'POST'])
def api_settings_branches():
    conn = get_db()
    c = conn.cursor()
    try:
        if request.method == 'GET':
            include_inactive = request.args.get('all', '0') == '1'
            if include_inactive:
                c.execute('SELECT id, name, active FROM branches ORDER BY name')
            else:
                c.execute('SELECT id, name, active FROM branches WHERE active = 1 ORDER BY name')
            return jsonify([{'id': r[0], 'name': r[1], 'active': bool(r[2])} for r in c.fetchall()])
        ...
        c.execute('INSERT OR IGNORE INTO branches (name, active) VALUES (?, 1)', (name,))
        c.execute('UPDATE branches SET active = 1 WHERE name = ?', (name,))
        conn.commit()
        return jsonify({'status': 'success'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
@app.route('/api/settings/branches/<int:item_id>', methods=['PUT', 'DELETE'])
def api_settings_branches_detail(item_id):
    conn = get_db()
    c = conn.cursor()
    try:
        c.execute('SELECT name FROM branches WHERE id = ?', (item_id,))
        row = c.fetchone()
        old_name = row[0] if row else None
        ...
        if request.method == 'PUT':
            ...
            if new_name is not None:
                ...
                if not new_name:
                    return jsonify({'status': 'error', 'message': 'Nombre inválido'}), 400
                c.execute('UPDATE branches SET name = ? WHERE id = ?', (new_name, item_id))
                if old_name:
                    c.execute('UPDATE tickets SET branch = ? WHERE branch = ?', (new_name, old_name))

            if active is not None:
                c.execute('UPDATE branches SET active = ? WHERE id = ?', (1 if active else 0, item_id))

            conn.commit()
            return jsonify({'status': 'success'})

        c.execute('UPDATE branches SET active = 0 WHERE id = ?', (item_id,))
        conn.commit()
        return jsonify({'status': 'success'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
@app.route('/api/settings/agents', methods=['GET', 'POST'])
def api_settings_agents():
    conn = get_db()
    c = conn.cursor()
    try:
        if request.method == 'GET':
            include_inactive = request.args.get('all', '0') == '1'
            if include_inactive:
                c.execute('SELECT id, name, active FROM agents ORDER BY name')
            else:
                c.execute('SELECT id, name, active FROM agents WHERE active = 1 ORDER BY name')
            return jsonify([{'id': r[0], 'name': r[1], 'active': bool(r[2])} for r in c.fetchall()])
        ...
        c.execute('INSERT OR IGNORE INTO agents (name, active) VALUES (?, 1)', (name,))
        c.execute('UPDATE agents SET active = 1 WHERE name = ?', (name,))
        conn.commit()
        return jsonify({'status': 'success'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
@app.route('/api/settings/agents/<int:item_id>', methods=['PUT', 'DELETE'])
def api_settings_agents_detail(item_id):
    conn = get_db()
    c = conn.cursor()
    try:
        c.execute('SELECT name FROM agents WHERE id = ?', (item_id,))
        row = c.fetchone()
        old_name = row[0] if row else None
        ...
        if request.method == 'PUT':
            ...
            if new_name is not None:
                ...
                if not new_name:
                    return jsonify({'status': 'error', 'message': 'Nombre inválido'}), 400
                c.execute('UPDATE agents SET name = ? WHERE id = ?', (new_name, item_id))
                if old_name:
                    c.execute('UPDATE tickets SET agent = ? WHERE agent = ?', (new_name, old_name))

            if active is not None:
                c.execute('UPDATE agents SET active = ? WHERE id = ?', (1 if active else 0, item_id))

            conn.commit()
            return jsonify({'status': 'success'})

        c.execute('UPDATE agents SET active = 0 WHERE id = ?', (item_id,))
        conn.commit()
        return jsonify({'status': 'success'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 500
    ...
    try:
        ...
        if creation_date and close_date and close_date != 'None' and close_date != '':
            try:
                ...
                for fmt in ("%d/%m/%Y", "%Y-%m-%d", "%d-%m-%Y"):
                    try:
                        c_dt = datetime.strptime(str(creation_date), fmt)
                        cl_dt = datetime.strptime(str(close_date), fmt)
        ...
        if branch:
            ...
            if not c.fetchone():
                c.execute("INSERT INTO branches (name, active) VALUES (?, 1)", (branch,))
            c.execute("UPDATE branches SET active = 1 WHERE name = ?", (branch,))
        
        user = data.get('user')
        if user:
            c.execute("SELECT 1 FROM users WHERE name = ?", (user,))
            if not c.fetchone():
                c.execute("INSERT INTO users (name, active) VALUES (?, 1)", (user,))
            c.execute("UPDATE users SET active = 1 WHERE name = ?", (user,))
        
        agent = data.get('agent')
        if agent:
            agent = str(agent).strip()
            if agent:
                c.execute("SELECT 1 FROM agents WHERE name = ?", (agent,))
                if not c.fetchone():
                    c.execute("INSERT INTO agents (name, active) VALUES (?, 1)", (agent,))
                c.execute("UPDATE agents SET active = 1 WHERE name = ?", (agent,))
...
# Endpoint para escanear patrones usuario-sucursal
@app.route('/api/scan_user_patterns', methods=['POST'])
def scan_user_patterns():
    try:
        conn = get_db()
        c = conn.cursor()
        
        c.execute("""
            SELECT user, branch, COUNT(*) as frequency 
            FROM tickets 
            WHERE user IS NOT NULL AND TRIM(user) != '' 
            AND branch IS NOT NULL AND TRIM(branch) != ''
            GROUP BY user, branch
            ORDER BY user, frequency DESC
        """)
        
        patterns = c.fetchall()
        c.execute("DELETE FROM user_branch_patterns")
        
        for user, branch, frequency in patterns:
            confidence_score = min(1.0, frequency / 10.0)
            c.execute("""
                INSERT INTO user_branch_patterns 
                (user_name, branch_name, frequency, confidence_score)
                VALUES (?, ?, ?, ?)
            """, (user.strip(), branch.strip(), frequency, confidence_score))
        ...
        return jsonify({
            'status': 'success',
            'message': f'Se generaron {len(patterns)} patrones usuario-sucursal'
        })
        
    except Exception as e:
        return jsonify({
            'status': 'error',
            'message': f'Error al escanear patrones: {str(e)}'
        }), 500

# Endpoint para obtener sugerencias de sucursales para un usuario
@app.route('/api/suggest_branch/<user_name>', methods=['GET'])
def suggest_branch(user_name):
    try:
        conn = get_db()
        c = conn.cursor()
        
        c.execute("""
            SELECT branch_name, confidence_score, frequency 
            FROM user_branch_patterns 
            WHERE user_name = ? 
        ...
        if not patterns:
            return jsonify({
                'status': 'success',
                'suggestions': [],
                'message': 'No se encontraron patrones para este usuario'
            })
        ...
        suggestions = [
            {
                ...
                'frequency': frequency,
                'description': f'{frequency} tickets ({int(confidence * 100)}% confianza)'
            }
            for branch, confidence, frequency in patterns
        ]
...
def dashboard_stats():
    try:
        ...
        c.execute("SELECT COUNT(*) FROM tickets WHERE status = 'Cerrado'")
        closed_tickets = c.fetchone()[0]
        
        c.execute("SELECT COUNT(*) FROM tickets WHERE status = 'Abierto'")
        open_tickets = c.fetchone()[0]
        
        c.execute("SELECT COUNT(*) FROM tickets WHERE status = 'En Progreso'")
        in_progress_tickets = c.fetchone()[0]
        ...
        c.execute("""
            ...
            AND CAST(SUBSTR(delay, 1, INSTR(delay, ' ') - 1) AS INTEGER) <= 3
        """)
        sla_compliant = c.fetchone()[0]
        
        sla_compliance_rate = (sla_compliant / closed_tickets * 100) if closed_tickets > 0 else 0
        ...
        """)
        last_month_tickets = c.fetchone()[0]
...
# Endpoint para tendencias de tickets
...
def dashboard_trends():
    try:
        ...
        c.execute("""
            SELECT 
                strftime('%Y-%W', creation_date) as week,
                COUNT(*) as tickets_count,
                SUM(CASE WHEN status = 'Cerrado' THEN 1 ELSE 0 END) as closed_count
            FROM tickets 
            WHERE creation_date >= date('now', '-12 weeks')
            GROUP BY strftime('%Y-%W', creation_date)
            ORDER BY week ASC
        """)
...
# Endpoint para KPIs de SLA
@app.route('/api/dashboard/sla', methods=['GET'])
def dashboard_sla():
    try:
        conn = get_db()
        c = conn.cursor()
        
        # SLA por rango de tiempo
        c.execute("""
            SELECT 
                CASE 
                    WHEN CAST(SUBSTR(delay, 1, INSTR(delay, ' ') - 1) AS INTEGER) <= 1 THEN '≤ 1 día'
                    WHEN CAST(SUBSTR(delay, 1, INSTR(delay, ' ') - 1) AS INTEGER) <= 3 THEN '2-3 días'
                    WHEN CAST(SUBSTR(delay, 1, INSTR(delay, ' ') - 1) AS INTEGER) <= 7 THEN '4-7 días'
                    ELSE '> 7 días'
                END as sla_range,
                COUNT(*) as count
            FROM tickets 
        ...
        c.execute("""
            SELECT 
                ...
                COUNT(*) as total,
                SUM(CASE 
                    WHEN CAST(SUBSTR(delay, 1, INSTR(delay, ' ') - 1) AS INTEGER) <= 3 
                    THEN 1 ELSE 0 
                END) as compliant
            FROM tickets 
            WHERE close_date IS NOT NULL 
        ...
        sla_by_branch = []
        for row in c.fetchall():
            compliance_rate = (row[2] / row[1] * 100) if row[1] > 0 else 0
            sla_by_branch.append({
                'branch': row[0],
                'total': row[1],
                'compliant': row[2],
        ...
        return jsonify({
            'status': 'error',
            'message': f'Error al obtener SLA: {str(e)}'
        }), 500

# Endpoint para mapa de calor
...
def dashboard_heatmap():
    try:
        ...
        c.execute("""
            SELECT 
                CAST(strftime('%H', creation_date) AS INTEGER) as hour,
                COUNT(*) as tickets_count
            FROM tickets 
            WHERE creation_date IS NOT NULL 
            AND creation_date >= date('now', '-30 days')
        ...
        c.execute("""
            SELECT 
                CAST(strftime('%w', creation_date) AS INTEGER) as day_of_week,
                CASE CAST(strftime('%w', creation_date) AS INTEGER)
                    WHEN 0 THEN 'Domingo'
                    WHEN 1 THEN 'Lunes'
                    WHEN 2 THEN 'Martes'
            ...
            WHERE creation_date IS NOT NULL 
            AND creation_date >= date('now', '-30 days')
            GROUP BY strftime('%w', creation_date)
        ...
        for row in c.fetchall():
            daily_heatmap.append({
                'day_of_week': int(row[0]) if row[0] is not None else 0,
                'day_name': row[1] if row[1] is not None else 'Desconocido',
                'count': row[2]
            })
        
        # Top sucursales por actividad
        c.execute("""
            SELECT 
                branch,
                COUNT(*) as tickets_count
        ...
        for row in c.fetchall():
            top_branches.append({
                'branch': row[0] if row[0] is not None else 'Sin sucursal',
                'count': row[1]
            })
        ...
        for row in c.fetchall():
            top_users.append({
                'user': row[0] if row[0] is not None else 'Sin usuario',
                'count': row[1]
            })
        ...
        return jsonify({
            'status': 'error',
            'message': f'Error al obtener mapa de calor: {str(e)}'
        }), 500
...
def api_add_server():
    ...
    try:
        ...
        if branch_name:
            ...
            if branch_name:
                c.execute("INSERT OR IGNORE INTO branches (name, active) VALUES (?, 1)", (branch_name,))
                c.execute("UPDATE branches SET active = 1 WHERE name = ?", (branch_name,))
        ...
        c.execute('''INSERT INTO servers (branch, branch_code, dyndns, primary_service_provider, 
                      ...
                      secondary_service_ip, secondary_service_speed, category) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
